...
def arrowplot(path, max_LU=0, essential=[], CEN=[], names="", essential_col=True, filename="Arrowplot", REF=False, Show=True):
    ...
    if not os.path.exists("arrow_plots"):
        os.makedirs("arrow_plots")

    # get discrete colormap
    # Find the biggest LUs in the list.
    if max_LU == 0:
    ...
    # Choose the colour gradient. I like gist_rainbow and jet. jet might be a little be better for color blindness. Other options are: jet, gist_rainbow, hsv, viridis, plasma, magma, gnuplot2, rainbow
    ...
    for i in range(len(path)):  # y-axis
        ...
        for j in range(len(path[i])):  # x-axis
            ...
            if path[i][j] >= 0:
                ...
                triangle = plt.Polygon([[j, i+0.25], [j, i-0.25], [j+1, i]], facecolor=colour, edgecolor=edgecolor, label=abs(path[i][j]), linewidth=linewidth)
                plt.gca().add_patch(triangle)
            else:
                #plt.scatter(j, i, color=colour, marker="<", s=100, zorder=-1, edgecolor="dimgrey", label=abs(path[i][j]))
                #triangle = plt.Polygon([[j+0.5,i+0.25], [j+0.5,i-0.25], [j-0.5,i]], facecolor=colour, edgecolor="dimgrey")
                triangle = plt.Polygon([[j+1, i+0.25], [j+1, i-0.25], [j, i]], facecolor=colour, edgecolor=edgecolor, label=abs(path[i][j]), linewidth=linewidth)
                plt.gca().add_patch(triangle)
...
if __name__ == '__main__':
    ...
    #arrowplot([synIXR], max_LU=44, essential=essential, CEN=CEN, names=["synIXR"], essential_col=False, filename="synIXR0", REF=False)
    #arrowplot([synIXR, JS731_SR, JS731_LR], max_LU=44, essential=essential, CEN=CEN, names=names, essential_col=False, filename="JS731", REF=False)
